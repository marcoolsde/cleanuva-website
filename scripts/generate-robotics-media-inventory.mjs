...
const bytesOf = (dir, f) => {
  try {
    return statSync(join(ROOT, dir, f)).size;
  } catch {
    return 0;
  }
};
const fmtSize = (n) => (n >= 1024 * 1024 ? `${(n / 1048576).toFixed(1)} MB` : `${Math.round(n / 1024)} KB`);
...
// Which robotics-image basenames does any USAGE row reference? Anything on disk
// that nobody references is surfaced below as a cleanup candidate (kept on disk
// on purpose for now — not auto-deleted).
const IMG_WEB = `/${IMG_DIR.replace(/^public\//, "")}`; // "/images/robotics"
const referenced = new Set(
  Object.values(USAGE)
    .flat()
    .map((r) => r.path)
    .filter((p) => p.startsWith(`${IMG_WEB}/`))
    .map((p) => p.split("/").pop()),
);
const unused = imgs.filter((f) => !referenced.has(f));

const fileLine = (dir) => (f) => `- \`${f}\` — ${fmtSize(bytesOf(dir, f))}`;
...
${imgs.length ? imgs.map(fileLine(IMG_DIR)).join("\n") : "- (none)"}

## Files on disk — \`${VID_DIR}/\`
${vids.length ? vids.map(fileLine(VID_DIR)).join("\n") : "- (none)"}
...
${unused.length ? unused.map(fileLine(IMG_DIR)).join("\n") : "- (none — every file is referenced)"}
